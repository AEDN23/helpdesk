...
function Q_row($sql = null)
{
    ...
    if ($sql === null) {
        ...
        if ($result = $db->query($sql)) {
            $row = $result->fetch_assoc(); // Mengambil satu baris data sebagai array asosiatif
            $result->free(); // Membersihkan memori
            $db->close(); // Menutup koneksi
            return $row; // Mengembalikan baris data
        } else {
            $db->close(); // Jika query gagal, tetap tutup koneksi
