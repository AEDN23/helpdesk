...
if (isset($_POST['save'])) {
    ...
    $result = Q_row($query);
    $totalRows = $result['total_rows']; // Menghitung total data saat ini
    $nextId = $totalRows + 1; // Menentukan urutan berikutnya
    $tt_no_id = "ITHDK" . "-" . str_pad($nextId, 3, '0', STR_PAD_LEFT);

    echo $tt_no_id; // Contoh output: ITHDK-001, ITHDK-002, dst.


    $sql = "INSERT INTO tbl_ticket (tt_no_id, tt_user, tt_subject, tt_department, tt_service, tt_priority, tt_message) 
            VALUES ('$tt_no_id', '$a', '$b', '$c', '$d', '$e', '$f')";
    if (Q_execute($sql)) {
